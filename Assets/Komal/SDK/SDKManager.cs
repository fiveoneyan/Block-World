...
namespace komal.sdk
{
    public class SDKManager : Singleton<SDKManager>, ILifeCycle, IDebugLog, IAD, IGameCenter, IIAP, IAnalytics, IPush, IAR
    {
        ...
        #region App内推模块
        private IAR m_ProxyAR = null;
        private bool m_HasBanner = false;
        private bool m_NeedShowAR = true;
        ...
        public void ShowRecommend(Action<RecommendResult> callback)
        {
            ...
            if (IsPurchasing() || IsRestorePurchasing() || IsDisplayingAD() || IsAdsRemoved())
            {
                return;
            }

            m_ProxyAR.ShowRecommend((result) =>
            {
                callback(result);
                bool banner = m_HasBanner;
                if (result == RecommendResult.DISPLAY)
                {
                    banner = m_HasBanner;
                    HideBanner();
                }
                else if (result == RecommendResult.DISMISS)
                {
                    if (banner)
                        ShowBanner();
                }
        ...
        {
            m_NeedShowAR = false;
            m_ProxyGameCenter.OpenGameCenter();
        }
        ...
        public bool IsRestorePurchasing()
        {
            if (m_ProxyIAP != null)
            {
                return m_ProxyIAP.IsRestorePurchasing();
            }
            else
            {
                return false;
...
#endif
                //app内推模块
                var appRecommendSDK = new AppRecommendSDK();
                m_SDKList.Add(appRecommendSDK);
                m_ProxyAR = appRecommendSDK;
